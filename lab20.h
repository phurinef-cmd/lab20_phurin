...
vector<int> Equipment::getStat(){
	vector<int> Eqstat;
	Eqstat.push_back(hpmax);
	Eqstat.push_back(atk);
	Eqstat.push_back(def);
	return Eqstat;
}
void Unit::equip(Equipment *eq){
	if(Unit::equipment != NULL){
		vector<int> oldeqstat = Unit::equipment->getStat();
		hpmax -= oldeqstat[0];
		atk -= oldeqstat[1];
		def -= oldeqstat[2];
	}
	vector<int> plusstat = eq->getStat();
	hpmax += plusstat[0];
	atk += plusstat[1];
	def += plusstat[2];
	equipment = eq;
